...
do
	echo "$NMSG mensagens de tamanho $ARGTAM com 8 processos" >> resultados.csv
	for vez in $(seq 1 10)
	do
		mpirun -np 8 --hostfile hostfile8.txt ./meuBcast "$NMSG" "$ARGTAM" -r 0 | grep 'Tempo:' | awk '{printf "%s\n",$2}' >> resultados.csv
	done

	echo "$NMSG mensagens de tamanho $ARGTAM com 16 processos" >> resultados.csv
	for vez in $(seq 1 10)
	do
		mpirun -np 16 --hostfile hostfile16.txt ./meuBcast "$NMSG" "$ARGTAM" -r 0 | grep 'Tempo:' | awk '{printf "%s\n",$2}' >> resultados.csv
	done

	echo "$NMSG mensagens de tamanho $ARGTAM com 32 processos" >> resultados.csv
	for vez in $(seq 1 10)
	do
		mpirun -np 32 --hostfile hostfile32.txt ./meuBcast "$NMSG" "$ARGTAM" -r 0 | grep 'Tempo:' | awk '{printf "%s\n",$2}' >> resultados.csv
	done																																	

	echo "$NMSG mensagens de tamanho $ARGTAM com 64 processos" >> resultados.csv
	for vez in $(seq 1 10)
	do
